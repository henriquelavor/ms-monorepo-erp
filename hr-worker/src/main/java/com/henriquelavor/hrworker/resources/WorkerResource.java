 ...
 * 
 * import com.henriquelavor.hrworker.entities.Worker; import
 * com.henriquelavor.hrworker.repositories.WorkerRepository;
 * 
 * @RefreshScope
 * 
 * @RestController
 * 
 * @RequestMapping(value = "/workers") public class WorkerResource {
 * 
 * private Logger logger = LoggerFactory.getLogger(WorkerResource.class);
 * 
 * @Value("${test.config}") private String testConfig;
 * 
 * @Autowired private Environment env; // esta variavel tem varios informacoes
 * do contexto da aplicacao, como variaveis de configuracao etcc.
 * 
 ...
 * ResponseEntity.noContent().build(); //para ser uma resposta sem corpo }
 * 
 * //ResponseEntity encapsula resposta Http
 * 
 * @GetMapping public ResponseEntity<List<Worker>> findAll(){ List<Worker> list
 ...
 * logger.info("PORT = " + env.getProperty("local.server.port"));
 * 
 * Worker obj = repository.findById(id).get(); return ResponseEntity.ok(obj); }
 * 
 * 
 * 
 * }
